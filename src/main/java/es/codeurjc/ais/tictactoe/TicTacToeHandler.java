				...
				if (game.checkTurn(msg.data.playerId)) {
					...
					if(game.checkWinner().win) { 
						...
						for(Player p: game.getPlayers()) {
							if(!statistics.getStatistics().contains(p)) {
								statistics.getStatistics().add(p);
							}
							else {
								//coges el jugador del array statistics y le sumas las partidas ganadas y perdidas a las que ya tiene
								statistics.getStatistics().get(statistics.getStatistics().indexOf(p)).setPganadas(statistics.getStatistics().get(statistics.getStatistics().indexOf(p)).getPganadas()+p.getPganadas());
								statistics.getStatistics().get(statistics.getStatistics().indexOf(p)).setPperdidas(statistics.getStatistics().get(statistics.getStatistics().indexOf(p)).getPperdidas()+p.getPperdidas());
							}
						}
						
						//statistics.getStatistics().addAll(game.getPlayers());
					}
					if(game.checkDraw()) {
						for(Player p:game.getPlayers()) {
							p.anadirEmpatadas();
							if(!statistics.getStatistics().contains(p)) {
								...
								statistics.getStatistics().get(statistics.getStatistics().indexOf(p)).anadirEmpatadas();
							}
						}
					}
				}
